...
const _sfc_main = {
  ...
  setup(__props) {
    ...
    const filteredOrders = computed(
      () => orders.value.filter(
        (o) => o.code.toLowerCase().includes(searchQuery.value.toLowerCase()) && (filterStatus.value === "" || o.status === filterStatus.value)
      )
    );
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      _push(`<main class="flex-1 p-6"><section class="bg-white rounded-xl p-5 shadow"><div class="flex justify-between items-center mb-2"><h3 class="font-semibold text-gray-700 text-[20px]">Danh sách đơn hàng</h3><div class="flex items-center gap-3"><div class="h-[45px] px-4 flex items-center border border-gray-300 rounded-[10px] bg-transparent gap-2"><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5 text-gray-400"><path stroke-linecap="round" stroke-linejoin="round" d="M21 21l-4.35-4.35m0 0A7.5 7.5 0 1010.5 18a7.5 7.5 0 006.15-3.35z"></path></svg><input${ssrRenderAttr("value", searchQuery.value)} type="text" placeholder="Tìm kiếm đơn hàng..." class="flex-1 bg-transparent outline-none placeholder:text-gray-400"></div><div class="relative"><button class="h-[45px] px-4 flex items-center justify-between border border-gray-300 rounded-[10px] bg-transparent cursor-pointer w-[180px]"><span class="text-gray-600 text-sm">${ssrInterpolate(filterStatus.value || "Tất cả trạng thái")}</span><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5 text-gray-500"><path stroke-linecap="round" stroke-linejoin="round" d="M6 9l6 6 6-6"></path></svg></button>`);
      if (showFilter.value) {
        _push(`<div class="absolute top-[48px] left-0 w-full bg-white border border-gray-200 rounded-[10px] shadow-md overflow-hidden z-20"><div class="px-4 py-2 hover:bg-gray-100 cursor-pointer text-sm">Tất cả trạng thái</div><div class="px-4 py-2 hover:bg-gray-100 cursor-pointer text-sm">Đang chờ</div><div class="px-4 py-2 hover:bg-gray-100 cursor-pointer text-sm">Đang xử lý</div><div class="px-4 py-2 hover:bg-gray-100 cursor-pointer text-sm">Hoàn thành</div><div class="px-4 py-2 hover:bg-gray-100 cursor-pointer text-sm">Đã hủy</div></div>`);
      } else {
        _push(`<!---->`);
      }
      _push(`</div></div></div><hr class="border-t border-gray-200 mb-4"><div class="grid grid-cols-5 text-[#6E4E37] font-semibold text-sm px-2 mb-3"><div>#Mã đơn</div><div>Ngày mua</div><div>Tổng</div><div>Trạng thái</div><div>Hành động</div></div><div class="space-y-2"><!--[-->`);
      ssrRenderList(paginatedOrders.value, (order, i) => {
        _push(`<div class="h-[55px] grid grid-cols-5 items-center bg-[#F5F7FA] h-[55px] rounded-[10px] px-3 text-sm hover:bg-[#ECEFF3] transition"><div class="text-[#A77A5D] font-semibold cursor-pointer">${ssrInterpolate(order.code)}</div><div>${ssrInterpolate(order.date)}</div><div>${ssrInterpolate(order.total)}</div><div><span class="${ssrRenderClass({
...
//# sourceMappingURL=orders-BkrQ5jkd.mjs.map
