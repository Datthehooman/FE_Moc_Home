<template>
  <div class="w-[411px] h-135">
    <div class="w-full h-70 rounded-xl overflow-hidden relative mb-[9px]">
      <NuxtImg src="/blog 1.png" class="w-full h-full object-cover z-0" alt="Blog Image" />
      <div
    ...
    <NuxtLink :to="`/articles/${article.slug}`" class="block hover:text-primary transition-colors duration-300">
...
const contentSnippet = computed(() => {
    ...
    const cleanContent = (props.article.content || '').replace(/<[^>]*>/g, '').trim();
