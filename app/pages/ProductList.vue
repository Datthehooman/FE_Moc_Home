...
const filteredProducts = computed(() => {
  ...
  if (sortOption.value === 'Tên A-Z') {
    result = [...result].sort((a, b) => a.product_name.localeCompare(b.product_name, 'vi', { sensitivity: 'base' }))
  } else if (sortOption.value === 'Tên Z-A') {
    result = [...result].sort((a, b) => b.product_name.localeCompare(a.product_name, 'vi', { sensitivity: 'base' }))
...
watch(searchQuery, (keyword) => {
  ...
  timeout = setTimeout(async () => {
    ...
    try {
      ...
      if (keyword) {
        url = `http://127.0.0.1:8000/api/client/product-search?keyword=${encodeURIComponent(keyword)}`
      }
      ...
      products.value = list.map((p: any) => ({
        ...
        thumbnail:
          p.thumbnail ?? (p.images?.[0]?.image_url ? `http://127.0.0.1:8000/storage/${p.images[0].image_url}` : '/placeholder.png'),
      }))
    } catch (err) {
