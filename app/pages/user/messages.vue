        ...
        <section class="bg-white rounded-xl shadow p-6 flex gap-6 h-[650px] min-w-0">
          ...
          <aside class="w-[35%] border-r pr-4 shrink-0">
            ...
            <div class="flex items-center gap-3 p-3 rounded-xl bg-[#F5ECE5] min-w-0">
              <div class="w-10 h-10 rounded-full bg-[#6E4E37] text-white flex items-center justify-center font-bold shrink-0">M</div>
              <div class="flex flex-col min-w-0">
                <span class="font-medium text-sm">Mộc Home</span>
                <span class="text-xs text-gray-500 truncate">
                  {{ messages.length ? messages[messages.length-1]?.message || '📷 Hình ảnh' : 'Chưa có tin nhắn' }}
                </span>
              </div>
            </div>
          </aside>
          ...
          <div class="flex flex-col w-[100%] h-full min-w-0">
            ...
            <div class="flex items-center justify-between pb-4 border-b shrink-0">
              ...
              <div class="w-10 h-10 rounded-full bg-[#6E4E37] text-white flex items-center justify-center font-bold">M</div>
            </div>

            <!-- MESSAGES -->
            <div ref="messagesContainer" class="flex-1 overflow-y-auto py-4 space-y-4 custom-scroll min-w-0">
              <div v-for="msg in messages" :key="msg.id" class="flex min-w-0 items-start"
                ...
                <div class="bubble-safe px-3 py-2 rounded-2xl text-sm flex flex-col gap-1 max-w-[70%]"
                  ...
                  <div v-if="msg.image_url" class="flex flex-wrap gap-2 mt-1">
                    <img :src="msg.image_url" class="w-[100px] h-[100px] object-cover rounded-lg cursor-pointer" @click="openPreview(msg.image_url)" />
                  </div>
            ...
            <div class="pt-3 border-t shrink-0">
              <div v-if="previewImages.length" class="flex flex-wrap gap-2 mb-2">
                <div v-for="(img, i) in previewImages" :key="i" class="relative">
                  <img :src="img" class="w-[72px] h-[72px] object-cover rounded-lg cursor-pointer" @click="openPreview(img)" />
              ...
              <div class="flex items-center gap-2 bg-[#F0F2F5] rounded-2xl px-3 py-2 min-w-0">
                <label class="cursor-pointer shrink-0">
                  <UIcon name="i-heroicons-camera-20-solid" class="size-6 text-gray-500" />
                ...
                <button @click="handleSendMessage" :disabled="!newMessage.trim() && !selectedImages.length" class="text-[#0084FF] disabled:text-gray-400 shrink-0">
...
onMounted(async () => {
  ...
  if (convo?.id) {
    ...
    await loadMessages()
    startPolling() // tự load khi có tin nhắn mới
  }
})
...
function startPolling() {
  pollingInterval = setInterval(loadMessages, 3000) // 3 giây kiểm tra tin nhắn mới
}
...
function handleInput() {
  ...
  el.style.height = h <= 80 ? h + 'px' : '80px'
}
...
async function handleSendMessage() {
  if (!conversationId.value || (!newMessage.value.trim() && !selectedImages.length)) return
  await sendMessage(conversationId.value, { message: newMessage.value || undefined, images: selectedImages.value })
  newMessage.value = ''
