  ...
  <transition name="fade-slide">
    ...
    >
      <div class="py-1">
        ...
        <!-- Input lý do khác -->
       <!-- Input lý do khác -->
<div v-if="filterStatus === 'Lý do khác'" class="px-4 py-2">
  <input
    v-model="cancelReason"
    type="text"
    placeholder="Nhập lý do hủy"
    class="w-full border border-gray-300 rounded-md px-2 py-1 text-sm focus:outline-none focus:border-[#A77A5D]"
  />
</div>
...
<div class="bg-gray-50 p-6 rounded-xl border border-gray-200">
  ...
  <template v-if="invoiceLoading">
    <p class="text-gray-500 text-sm">Đang tải hóa đơn...</p>
  </template>
  ...
  <template v-else-if="invoice">
    <dl class="text-sm space-y-2">
      ...
      <div class="flex justify-between">
        ...
        <dd class="font-medium text-gray-700">{{ invoice.payment_method?.name || '-' }}</dd>
      </div>
      ...
      <div class="flex justify-between">
        ...
        <span>- {{ Number(invoice.discount_amount).toLocaleString('vi-VN') }} ₫</span>
      </div>
      ...
      <div class="flex justify-between">
        ...
        <dd class="font-medium text-gray-700">
          + {{ Number(invoice.shipping_fee) === 0 ? 'Free' : Number(invoice.shipping_fee).toLocaleString('vi-VN') + ' ₫' }}
        </dd>
      </div>

      <!-- Cọc nếu có -->
      <div v-if="Number(invoice.deposit_amount) > 0" class="flex justify-between">
        <dt class="text-gray-600">Tiền cọc:</dt>
        <dd class="font-medium text-gray-700">
          {{ Number(invoice.deposit_amount).toLocaleString('vi-VN') }} ₫
        </dd>
      </div>

      <!-- Số tiền còn lại -->
      <div class="flex justify-between">
        <dt class="text-gray-600">Số tiền cần thanh toán:</dt>
        <dd class="font-medium text-gray-700">
      ...
      <div class="flex justify-between border-t border-gray-300 pt-3 mt-3 text-lg font-bold">
        ...
        <dd class="text-[#A77A5D]">{{ Number(invoice.total_amount).toLocaleString('vi-VN') }} ₫</dd>
      </div>

      <!-- Thông tin khách hàng -->
      <!-- <div class="mt-4 border-t border-gray-200 pt-3">
        <h5 class="font-semibold text-gray-700 mb-2">Thông tin khách hàng</h5>
        <p class="text-gray-600 text-sm">Họ tên: {{ invoice.customer_info?.name }}</p>
        <p class="text-gray-600 text-sm">Số điện thoại: {{ invoice.customer_info?.phone }}</p>
        <p class="text-gray-600 text-sm">Email: {{ invoice.customer_info?.email }}</p>
        <p class="text-gray-600 text-sm whitespace-pre-wrap">Địa chỉ: {{ invoice.customer_info?.address }}</p>
      </div> -->
    </dl>
  </template>
...
const statusText = computed(() => statusTextMap[order.value?.status] || "Không xác định");

// --- Dropdown chọn lý do hủy ---
const applyFilter = (val: string) => {
  filterStatus.value = val;
  if (val !== "Lý do khác") {
    cancelReason.value = val; // tự động set lý do nếu không phải "Lý do khác"
  } else {
    cancelReason.value = ""; // để người dùng nhập
  }
  // không đóng dropdown ngay
...
const handleCancel = async () => {
  ...
  await cancelOrder(cancelReason.value);
  await fetchOrderDetail(id); // reload chi tiết order
...
// --- onMounted: load order + invoice ---
onMounted(async () => {
  await fetchOrderDetail(id);
  await fetchInvoice(id); // gọi API lấy hóa đơn
});
</script>
